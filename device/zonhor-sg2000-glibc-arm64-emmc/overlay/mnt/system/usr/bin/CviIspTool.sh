...
# IMX678 detected: pick RTSP cfg + mode-specific ISP PQ bin
if [ -f "$SENSOR_CFG" ] && grep -q 'SONY_IMX678' "$SENSOR_CFG" 2>/dev/null; then
	...
	if grep -q 'SONY_IMX678_MIPI_2M_30FPS_10BIT_BIN' "$SENSOR_CFG" 2>/dev/null; then
		ISP_BIN="${PARAM_DIR}/cvi_sdr_bin_IMX678_1080P_BIN"
	elif grep -q 'SONY_IMX678_MIPI_8M_30FPS_12BIT' "$SENSOR_CFG" 2>/dev/null; then
		ISP_BIN="${PARAM_DIR}/cvi_sdr_bin_IMX678_5M"
	else
		ISP_BIN="${PARAM_DIR}/cvi_sdr_bin_IMX678_1080P_BIN"
	fi
	if [ -f "$ISP_BIN" ]; then
		ln -sfn "$(basename "$ISP_BIN")" "${PARAM_DIR}/cvi_sdr_bin" 2>/dev/null || true
		echo "IMX678: ISP bin -> $ISP_BIN"
	fi
fi
