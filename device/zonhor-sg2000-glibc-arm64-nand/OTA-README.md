# Zonhor SG2000 SPI NAND A/B OTA 更新说明
...
```bash
./build.sh zonhor-sg2000-glibc-arm64-nand
# Windows/USB 烧录请使用 out/zonhor-sg2000-glibc-arm64-nand_<时间戳>.zip
# 解压后必须包含 fip.bin、boot.nand、boot_b.nand、rootfs_ext4.nand、rootfs_b_ext4.nand
```

不要使用 `out/zonhor-sg2000-glibc-arm64-nand_ota_<时间戳>.zip` 做 USB 烧录；OTA 包会省略 B 槽镜像，并包含 `DONT_CARE` sparse chunk，当前 U-Boot USB 下载路径不保证支持。
...
- `out/zonhor-sg2000-glibc-arm64-nand_<时间戳>.zip`：USB 工厂/救砖烧录包，包含 A/B 两槽镜像，CIMG 保持 USB 兼容格式。
- `out/zonhor-sg2000-glibc-arm64-nand_ota_<时间戳>.zip`：板端 OTA 包，内含**单份** boot/rootfs，不含 BOOT_B/ROOTFS_B。
...
```bash
scp out/zonhor-sg2000-glibc-arm64-nand_ota_*.zip root@<设备IP>:/mnt/data/upgrade.zip
...
| 软重启停在 `Starting kernel` | 多为大块 SPI NAND 写后 warm reset；断电冷启可继续；脚本已改为 `sync`+`blockdev --flushbufs`+普通 `reboot` |
| 校验失败 | 重新上传完整 zip |
...
- 分区表：`build/boards/cv181x/sg2000_zonhor_sg2000_glibc_arm64_nand/partition/partition_nand.xml`
- U-Boot 选槽：`u-boot-2021.10/cmd/cvi_boot_mode_nand.c`
- 板级脚本：`device/zonhor-sg2000-glibc-arm64-nand/overlay/`
